...
(function ($) {
    ...
    var GesturePasswd = function (element, options) {
        ...
        this.initDraw = function () {
            this.$ctx.strokeStyle = this.color; //边框样式
            this.$ctx.lineWidth = 2;
            for (var j = 0; j < 3; j++) {
                for (var i = 0; i < 3; i++) {
                   this.$ctx.moveTo(this.o / 2 + this.rr * 2 + i * (this.o + 2 * this.rr), this.o / 2 + this.rr + j * (this.o + 2 * this.rr));//绘制线段
                    //this.$ctx.beginPath();
                    this.$ctx.arc(this.o / 2 + this.rr + i * (this.o + 2 * this.rr), this.o / 2 + this.rr + j * (this.o + 2 * this.rr), this.rr, 0, 2 * Math.PI); //绘制圆弧
                    //this.$ctx.closePath();

                    var tem = new Point(this.o / 2 + this.rr + i * (this.o + 2 * this.rr), this.o / 2 + this.rr + j * (this.o + 2 * this.rr)); //创建Point
                    if (that.pList.length < 9)
                        this.pList.push(tem);
                }
            }
            this.$ctx.stroke(); //canvas元素绘制图像的时候有两种方法,绘制边框
